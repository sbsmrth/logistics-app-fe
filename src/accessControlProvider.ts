...
export const accessControlProvider: AccessControlProvider = {
  can: async ({ action, resource, params }) => {
    ...
    if (role === "GERENTE") {
      if (resource && ["users", "couriers"].includes(resource)) return { can: false };
      return { can: true };
    }

    // DESPACHADOR puede ver y gestionar orders, products y categories
    if (role === "DESPACHADOR") {
      if (resource && ["orders", "products", "categories", "dashboard"].includes(resource)) {
        return { can: true };
      }
      return { can: false };
    }
    ...
    if (role === "CLIENTE") {
      ...
      if (resource && ["products", "dashboard"].includes(resource)) {
        return { can: true };
      }
      return { can: false };
    }
